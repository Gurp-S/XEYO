...
for dp,dn,fn in os.walk(ROOT):
    ...
    for f in fn:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Try) and any(pass_only(h.body) for h in node.handlers):
                names=[call_name(n) for s in node.body for n in ast.walk(s) if isinstance(n, ast.Call)]
                obs=[x for x in names if is_obs(x)]
                if obs:

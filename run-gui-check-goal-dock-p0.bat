@echo off
REM XEYO 41 P0 GUI check: typecheck + vitest (goal dock batch).
...
pushd gui
call npm run typecheck > "..\logs\gui-typecheck-goal-dock.log" 2>&1
set RC1=%ERRORLEVEL%
popd
powershell -NoProfile -Command "Get-Content -LiteralPath 'logs\gui-typecheck-goal-dock.log' -Tail 25"
echo typecheck exit code: %RC1%
echo.

echo [XEYO] [2/2] vitest run (gui, full unit suite) ...
pushd gui
call npm test > "..\logs\vitest-goal-dock.log" 2>&1
set RC2=%ERRORLEVEL%
popd
powershell -NoProfile -Command "Get-Content -LiteralPath 'logs\vitest-goal-dock.log' -Tail 45"

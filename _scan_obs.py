...
for dp,dn,fn in os.walk(ROOT):
    ...
    for f in fn:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ExceptHandler) and pass_only(node.body):
                names=[call_name(n) for n in ast.walk(node) if isinstance(n, ast.Call)]
                if any(is_obs(x) for x in names):
                    rows.append((p,node.lineno,[x for x in names if is_obs(x)]))
print("OBS_BAREPASS_TOTAL", len(rows))

...
``run_task`` 流程：
1. 读 main head 作为 base_commit；
2. CAS 认领（pending / reopened 可认领，双进程下恰好一个成功）；
3. ``worktree add -b coord/task/<短名> <base>``——worker 会话 cwd 指向
   worktree（独立 index，不碰主仓 index.lock）；
4. 执行 ``work_fn(worktree_path)``（阶段 1 为注入回调；阶段 2 接
   agent_tool/subagent_runner 派生真会话）；
5. ``commit_all``（无改动 = 空 diff 也合法，committed=False）；
6. ``submit_result``（claimed → ready_to_merge，附分支名，交 reconciler）。
...
class WorkerPool:
    ...
    def run_task(self, task_id: str, worker_id: str,
        ...
        claimed = self.store.claim_task(task_id, worker_id, base)
        if claimed is None:

...
def _marked_names() -> list[str]:
	"""全文扫描：装配点统一走 `_tag_block(tagged, "名", ...)`（允许跨行）——
	登记名是真实代码参数（比注释强，改名即编译红）。裸 `tagged.append(`
	在本文件禁止。"""
	assert "tagged.append(" not in _SRC.replace(
		"\ttagged.append(block)", "", 1
...
def test_registry_entries_are_complete():
	for name, meta in T_NOW_BLOCK_REGISTRY.items():
		assert meta.get("klass") in {"directive", "event", "inventory"}, (
			f"{name}.klass 非法：{meta.get('klass')!r}"
		)
